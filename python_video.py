...
for translated_text in word_list:
    #translate text
    translator_object = translator.translate(translated_text, dest=translate_language)
    print(translated_text, " --> ", translator_object.text)
    translated_text = translator_object.text

    #make audio files for the translated text
    #original text slow
    myobj = gTTS(text=translated_text, lang=initial_language, slow=True)
    ...
    #original text
    myobj3 = gTTS(text=translated_text, lang=initial_language, slow=False)
